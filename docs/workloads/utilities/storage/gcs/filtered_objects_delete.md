...
This job allows the user to delete all objects in a bucket path based on the metadata that is set on them.
The user can choose to delete objects with specific metadata (matching the provided key and/or value),
...
Allows the job to perform a 'dry run' where objects are listed but not deleted.
To perform the actual deletion, user should untick this.

Note: deletion of objects is irreversible.
